...
float x = 0.0; //xcoord for animation
float upperarmAngle = 45;
float forearmAngle= -45;
float handAngle= -45;
...
void drawUnitFace(float r , float g, float b)
{
...
void drawUnitCube()
{
	...
	//drawUnitFace(1, 0, 0);
	glPushMatrix();
	drawUnitFace(1, 0, 0);        // front face is red
	glPopMatrix();

	glPushMatrix();
	glRotatef(180, 0, 1, 0 ); // rotate square to back face
	glTranslatef(-1,0,0);
	drawUnitFace(0, 1, 1);        // back face is cyan
	glPopMatrix();

	glPushMatrix();
	glRotatef(270, 0, 1, 0); // rotate square to left face
	glTranslatef(-0.5, 0, -0.5);
	drawUnitFace(0, 1, 0);        // left face is green
	glPopMatrix();

	glPushMatrix();
	glRotatef(90, 0, 1, 0); // rotate square to right face
	glTranslatef(-0.5, 0, 0.5);
	drawUnitFace( 1, 0, 1);       // right face is magenta
	glPopMatrix();

	glPushMatrix();
	glRotatef(-90, 1, 0, 0); // rotate square to top face
	glTranslatef(0, -0.5, 0.5);
	drawUnitFace(0, 0, 1);        // top face is blue
	glPopMatrix();

	glPushMatrix();
	glRotatef(90, 1, 0, 0); // rotate square to bottom face
	glTranslatef(0, -0.5, -0.5);
	drawUnitFace(1, 1, 0);        // bottom face is yellow
	glPopMatrix();
...
void drawUnitCube2()
{
	...
	//glTranslatef(x,y,z);
	glPushMatrix();
	drawUnitFace2();        // front face is red
	glPopMatrix();

	glPushMatrix();
	glRotatef(180, 0, 1, 0); // rotate square to back face
	glTranslatef(-1, 0, 0);
	drawUnitFace2();        // back face is cyan
	glPopMatrix();

	glPushMatrix();
	glRotatef(270, 0, 1, 0); // rotate square to left face
	glTranslatef(-0.5, 0, -0.5);
	drawUnitFace2();        // left face is green
	glPopMatrix();

	glPushMatrix();
	glRotatef(90, 0, 1, 0); // rotate square to right face
	glTranslatef(-0.5, 0, 0.5);
	drawUnitFace2();       // right face is magenta
	glPopMatrix();

	glPushMatrix();
	glRotatef(-90, 1, 0, 0); // rotate square to top face
	glTranslatef(0, -0.5, 0.5);
	drawUnitFace2();        // top face is blue
	glPopMatrix();

	glPushMatrix();
	glRotatef(90, 1, 0, 0); // rotate square to bottom face
	glTranslatef(0, -0.5, -0.5);
	drawUnitFace2();        // bottom face is yellow
	glPopMatrix();
...
void drawArm()
{
	...
	//glRotatef(-45, 0, 0, 1);
	//glScalef(1, 0.2, 0.2);
	glScalef(0.75*0.8, 0.1*0.8, 0.1*0.8);
	drawUnitCube();
	glPopMatrix();
	...
	glScalef(0.2*0.75*0.8, 1.5*0.08, 1.5*0.08);
	drawUnitCube2();
	glPopMatrix();
	glPopMatrix();
...
void drawMesh()
{
	...
	for (int i = 0; i < MeshVertices.size(); i+=9) {

		float P1 [3] = {MeshVertices[i], MeshVertices[i+1], MeshVertices[i+2]};
		float P2 [3] = { MeshVertices[i+3], MeshVertices[i + 4], MeshVertices[i + 5] };
		float P3 [3] = { MeshVertices[i+6], MeshVertices[i + 7], MeshVertices[i + 8] };

		float U[3] = { MeshVertices[i + 3] - MeshVertices[i] ,MeshVertices[i + 4]- MeshVertices[i + 1], MeshVertices[i + 5]- MeshVertices[i + 2] };
		float V[3] = { MeshVertices[i + 6] - MeshVertices[i] ,MeshVertices[i + 7] - MeshVertices[i + 1], MeshVertices[i + 8] - MeshVertices[i + 2] };
		...
		//glNormal3f(0, 0, 1);
		glVertex3f(MeshVertices[i+3], MeshVertices[i+4], MeshVertices[i+5]);
		//glNormal3f(0, 0, 1);
		glVertex3f(MeshVertices[i+6], MeshVertices[i+7], MeshVertices[i+8]);
		glEnd();
	}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'l':
		...
		break;
	case 'U':
		//increase upperarmAngle
		upperarmAngle += 1;
		break;
	case 'u':
		//decrease upperarmAngle
		upperarmAngle -= 1;
		break;
	case 'F':
		//increase forearmAngle
		forearmAngle += 1;
		break;
	case 'f':
		//decrease forearmAngle
		forearmAngle -= 1;
		break;
	case 'H':
		//increase handAngle
